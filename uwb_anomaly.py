...
class HybridMonitor:
    ...
    - 기본 설정은 AI 확률 기반 판단만 사용
    - 필요할 경우 FSM 규칙을 활성화할 수 있음
    """
    ...
    def __init__(
        ...
        ai_only: bool = True,
        fall_suspect_prob: float = 0.6,
        fall_confirm_prob: float = 0.85,
        unresp_confirm_prob: float = 0.7,
    ):
        self.fs = fs
        ...
        self.fall_suspect_prob = fall_suspect_prob
        self.fall_confirm_prob = fall_confirm_prob
        self.unresp_confirm_prob = unresp_confirm_prob
    ...
    def _apply_ai_only(self, fall_p: float, unresp_p: float) -> None:
        if unresp_p >= self.unresp_confirm_prob:
            self.state = State.UNRESPONSIVE
            return

        if fall_p >= self.fall_confirm_prob:
            self.state = State.FALL_CONFIRMED
            return

        if fall_p >= self.fall_suspect_prob:
    ...
    def update(self, frame: UWBFrame) -> Tuple[State, Dict[str, float]]:
        ...
        if self.ai_only:
            self._apply_ai_only(fall_p, unresp_p)
        else:
            # --- FSM: 넘어짐 의심 트리거(규칙) ---
            ...
            if self.state == State.FALL_SUSPECT:
                ...
                if still_for > self.fall_confirm_still_sec or fall_p > self.fall_confirm_prob:
                    self.state = State.FALL_CONFIRMED

            # 무반응: 오래 안 움직임 + (호흡 품질 낮거나) AI 확률 높음
            still_for = frame.t - (self.last_movement_t or frame.t)
            if still_for > self.unresp_still_sec and (
                unresp_p > self.unresp_confirm_prob or dbg["resp_quality"] < 0.2
            ):
                self.state = State.UNRESPONSIVE
...
def train_and_save_models(X: np.ndarray, y_fall: np.ndarray, y_unresp: np.ndarray) -> None:
    """
    X: (N, F) feature
    y_fall: (N,) 0/1
    y_unresp: (N,) 0/1
...
def run_demo_monitor(monitor: HybridMonitor, sleep_s: float = 0.05) -> None:
    while True:
...
def row_to_dict(row: Optional[tuple]) -> Optional[dict]:
    if row is None:
        return None
...
def contact_row_to_dict(row: Optional[tuple]) -> Optional[dict]:
    if row is None:
        return None
...
def notification_row_to_dict(row: Optional[tuple]) -> Optional[dict]:
    if row is None:
        return None
...
def update_status(event_id: int, new_status: EventStatus) -> None:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute(
        """
...
def update_notification_status(notification_id: int, new_status: NotificationStatus) -> None:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute(
        """
...
def notify_contact(event_row: dict, contact_row: dict) -> None:
    """
...
def trigger_119_call(event_row: dict) -> None:
    """
...
async def guardian_response(event_id: int, request: Request) -> dict:
    """
...
def list_notifications(event_id: int) -> list[dict]:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
...
async def update_contact(contact_id: int, request: Request) -> dict:
    data = await request.json()
    allowed_fields = {
        "name",
        "phone",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ai-only", action="store_true", default=True, help="AI 확률만으로 상태 판단")
    parser.add_argument("--no-ai-only", action="store_false", dest="ai_only", help="규칙+AI 혼합 판단")
    parser.add_argument("--fall-suspect-prob", type=float, default=0.6, help="넘어짐 의심 확률 임계값")
    parser.add_argument("--fall-confirm-prob", type=float, default=0.85, help="넘어짐 확정 확률 임계값")
    parser.add_argument("--unresp-confirm-prob", type=float, default=0.7, help="무반응 확정 확률 임계값")
...
def main() -> None:
    ...
    monitor = HybridMonitor(
        ...
        fall_suspect_prob=args.fall_suspect_prob,
        fall_confirm_prob=args.fall_confirm_prob,
        unresp_confirm_prob=args.unresp_confirm_prob,
    )
